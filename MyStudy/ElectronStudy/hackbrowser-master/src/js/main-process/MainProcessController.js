...
function MainProcessController() {
	...
	let init = function() {
        ...
        enablesocks5redirect();
		enablePepperFlashPlayer();
		attachEventHandlers();
	}

	let enablesocks5redirect =function(){
        ...
        app.commandLine.appendSwitch('--proxy-pac-url', 'file://'+global.__app.dataPath);
       console.log('file://'+global.__app.dataPath);
	}
	...
	let attachEventHandlers = function() {
		...
		app.on('ready', function() {
			...
			// check if .data directory exists 数据存储目录是否存在不存在则退出
